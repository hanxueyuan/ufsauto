...
def test_fio_config_seq_read():
    """测试顺序读配置"""
    config = FIOConfig(name="seq_read", rw="read", bs="128k", size="1G")
    assert config.rw == "read"


def test_fio_config_rand_write():
    """测试随机写配置"""
...
def test_fio_config_to_args_has_filename():
    """测试命令行包含 filename"""
...
def test_fio_config_to_args_has_direct():
    """测试命令行包含 direct"""
...
def test_fio_config_advanced_params():
    """测试高级参数"""
    config = FIOConfig(
        ramp_time=5,
        time_based=True,
...
def test_fio_config_various_block_sizes():
    """测试不同块大小"""
...
def test_fio_config_various_rw_types():
    """测试不同读写类型"""
...
def test_fio_init_timeout():
    """测试自定义超时"""
    fio = FIO(timeout=600)
    assert fio is not None


def test_fio_init_retries():
    """测试自定义重试"""
    fio = FIO(retries=3)
    assert fio is not None


def test_fio_has_run():
    """测试有 run 方法"""
    fio = FIO()
...
def test_fio_error_with_returncode():
    """测试 FIOError 带返回码"""
    err = FIOError("failed", returncode=1)
    assert err.returncode == 1


def test_fio_error_with_stderr():
