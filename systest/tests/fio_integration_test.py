...
try:
    ...
    test_info = runner._find_test("t_performance_SequentialReadBurst_001")
    fio_cmd = runner._build_fio_command("t_performance_SequentialReadBurst_001", 1, test_info)
    ...
    if json_start >= 0:
        ...
        # 测试结果解析
        parsed = runner._parse_fio_result(json_str, "t_performance_SequentialReadBurst_001")
    ...
    # 使用真实 FIO 数据测试验证逻辑
    test_cases = [
        ("t_performance_SequentialReadBurst_001", 2150.5, "PASS"),  # 高于目标 2100
        ("t_performance_SequentialReadBurst_001", 1900.0, "FAIL"),  # 低于目标 95%
    ...
    for test_name, bandwidth, expected in test_cases:
        result_data = {"status": "PASS", "metrics": {"bandwidth": bandwidth}}
            ...
            "test_cases": [
                {
                    ...
                    "status": "FAIL",
                    "metrics": {"bandwidth": 180.5, "latency_avg": 850.6, "latency_stddev": 650.3, "latency_p99999": 3500.0},
                }
            ]
        },
