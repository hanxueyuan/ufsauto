...
Postcondition:
    - 设备恢复：等待 5 秒回到空闲状态
    - 数据清理：删除临时文件和目录
    - 测试后状态检查：对比温度变化（Sustained 测试温升较大）、错误计数变化
...
测试参数:
    ...
    - iodepth: 32
    - numjobs: 1
    - runtime: 300（300 秒，Sustained 模式）
    - direct: 1
...
def main():
    ...
    success, output, error = run_fio(test_dir, FIO_PARAMS, job_name="seq_read_sustained")
    ...
    if changes["temp_rise"] and changes["temp_rise"] > 20:
        kit.info(f"⚠️ 温升较大 ({changes['temp_rise']}℃ > 20℃)，可能存在散热问题")
