...
测试模块：perf
测试用例 ID：t_perf_SeqReadSustained_002
测试优先级：P0
...
Test Steps:
    ...
    2. 使用 FIO 工具发起顺序读测试
       - 参数：rw=read, bs=128k, iodepth=32, numjobs=1, runtime=300, time_based, direct=1
...
Postcondition:
    ...
    - 测试后状态检查：
        ...
        - 重新读取温度，记录温度变化（Sustained 测试温升较大）
...
测试参数:
    ...
    - runtime: 300（测试时长：300 秒，Sustained 模式）
...
def collect_health_status(device):
    ...
    try:
        ...
        ufs_errors = [l for l in result.stdout.splitlines()
...
def run_fio(test_dir, fio_params):
    ...
    try:
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=fio_params["runtime"] + 120)
...
def main():
    """执行测试用例"""
    kit = Systest(
        test_name="t_perf_SeqReadSustained_002",
        device="/dev/ufs0",
    ...
    # Test Steps
    # ========================================
    kit.step("Test Steps - 执行 FIO 顺序读 Sustained 测试")
    ...
    # 温升检查（Sustained 测试温升较大）
    if health_before["temperature_c"] and health_after["temperature_c"]:
        temp_rise = health_after["temperature_c"] - health_before["temperature_c"]
        kit.info(f"温升: {temp_rise:.1f}℃")
        if temp_rise > 20:
            kit.info("⚠️ 温升较大 (>20℃)，可能存在散热问题")
