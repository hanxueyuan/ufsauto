...
Precondition:
    ...
    1.3 存储设备配置检查
        - Write Booster 状态：检查 /sys/block/<dev>/device/wb_enabled 或 sysfs 相关路径
          （Write Booster 对写入 Burst 性能影响显著，需特别关注）
...
def parse_fio_output(output):
    ...
    bw_match = re.search(r"WRITE:.*bw=(\d+(?:\.\d+)?)\s*(KiB|MiB|GiB)/s", output)
    if bw_match:
